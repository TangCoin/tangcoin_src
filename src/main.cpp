...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TangCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TangCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 4 * 60 * 60; // TangCoin: every 4 hours
static const int64 nTargetSpacing = 60; // TangCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // TangCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal tangcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TangCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "tang coin crypto";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388803430;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2718819;


        if (fTestNet)
        {
            block.nTime    = 1388773645;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("00b54b704cb23e386b06f4de8c111a4b5673a96f7ac3ba3c96247c11fc4362de"));
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // TangCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
